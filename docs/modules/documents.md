...
文档管理模块负责知识本身：把用户资料转化为可组织、可维护、可追溯的知识内容，并维护这些内容进入索引所需的状态。
...
- 对外提供带归属、版本和状态过滤的知识查询端口，供 Agent 组合成具体检索策略。
- 导出用户自己的资料。

用户可以在本模块完成资料收录、查看、编辑、归档、删除、专题标签整理、来源查看和版本回溯。导入成功不等于立即可检索：文档必须完成片段生成和索引刷新后才进入 `ready` 状态；更新会创建新版本，旧版本只用于历史引用和追溯。本模块向 Agent 提供候选片段查询，但不决定问题采用哪种检索策略。

## 边界

- 文档、专题、标签、收藏、归档、关联、来源、版本和索引状态归 documents 所有。
- 接收认证入口提供的可信身份，校验文档、专题、标签、关联对象及导出范围的归属，不允许模型或请求正文覆盖身份。
- 文档管理通过本模块入口对外提供；Agent 只能使用公开的知识查询端口，不访问文档内部数据。
...
文档模块中的“片段、知识点、实体、关系”分别表示不同层次的知识：片段是原文中可定位的检索单位，知识点是从片段中整理出的概念或结论，实体是人物、项目、技术等可被多个知识点引用的对象，关系描述知识点或实体之间的联系。它们都绑定用户和来源版本，删除或更新文档时可以追溯其来源；Agent 只读取这些对象，不负责创建文档领域数据。
...
### 知识查询端口（Knowledge Query）

知识查询端口负责在文档模块内部读取已经建立的索引，并按用户归属、文档状态和版本状态返回候选知识片段；它不决定本轮应该使用哪种检索方式，也不负责重排、证据判断和回答生成。检索策略属于 Agent，索引引擎的连接属于 infra。
...
type DocumentSearchPort interface {
    Search(query SearchQuery) ([]SearchHit, error) // 按归属、归档和 ready 版本过滤后返回候选，不承担 Agent 的策略判断。
