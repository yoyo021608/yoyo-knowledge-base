...
- 统一处理连接超时、资源释放和基础错误。
- 提供运行追踪，记录模型调用、检索步骤、耗时、Token 和失败原因，供 Agent 评估和故障排查使用。

业务模块通过本模块获得数据库事务、文件读写、全文和向量索引连接、模型调用、异步任务、缓存以及运行追踪。infra 的输出是技术资源、调用结果和执行记录，不是文档、回答、研究报告或学习结论。
...
- 运行追踪只记录技术执行事实，不判断回答是否正确；质量判断和评估结果由 Agent 负责，用户反馈由 sessions 保存。
