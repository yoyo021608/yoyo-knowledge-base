...
│           ├── storage.py
│           ├── jobs.py
│           ├── llm.py             # 模型 SDK 连接与轻量适配
│           ├── embeddings.py      # 向量生成 SDK 连接
│           └── migrations/
├── tests/
├── data/
├── scripts/
...
目录树说明职责归属；具体文件在对应职责进入实现时创建，模块目录与具体职责对应。`apps/api` 是启动壳，不是新的业务模块；业务真正归属于 `packages/backend` 下的领域包。
...
- `infra` 提供数据库、缓存、文件、任务和模型 SDK 连接；不反向依赖业务模块。向量查询规则归 documents，数据库连接归 infra。
...
~~~text
apps/api/app/main.py：创建应用、注入依赖、挂载 controller 路由
  → packages/backend/controller：HTTP 接入与模块调用协调
...
- apps/api 只负责启动、配置和依赖装配；业务逻辑与 HTTP 接口均归 packages/backend。
...
- Agent 的工具负责参数与结果适配，文档切分、索引、权限过滤、召回与融合归 documents；问题改写、证据判断、重排与上下文选择归 agent。
...
- controller 的协调函数由路由或后台任务调用，后台任务由 apps/api 注册并管理生命周期；任务调度归 infra，运行状态与恢复资格归 agent。
...
用户问题
  ↓
packages/backend/controller：解析请求并取得当前用户
  ↓
packages/backend/controller：校验会话归属和状态
  ↓
agent 公开接口：创建 queued Run
  ↓
packages/backend/controller：条件领取活动 Run；失败则标记 failed 并返回忙碌
  ↓
packages/backend/controller：按 Run 幂等保存 User Message，再启动执行
  ↓
agent/rag：问题改写与检索计划
  ↓
DocumentSearchPort
...
agent：持久化完整结果快照
  ↓
packages/backend/controller：BeginFinalize 领取提交权，sessions.SaveAnswer 幂等保存回答与引用，再完成 Run 并释放关联
~~~
...
- **为什么保留 Python 后端并把业务放入 packages**：FastAPI 负责 HTTP 接入，Python 领域包负责业务实现；`apps/api + packages/backend` 分离启动装配与领域逻辑，前端采用 React 和 TypeScript。
...
controller读取会话历史并交给 Agent。Agent 结合问题和会话上下文改写查询，再通过 `DocumentSearchPort` 请求 documents 做关键词、全文和向量混合召回。documents 负责切分、索引、权限/版本过滤、候选去重和结果融合；Agent 负责重排、证据判断、上下文预算和是否重查。证据门槛不通过时有限次改写和重检索，仍不足则返回证据不足；服务异常或超时单独返回故障，不混为检索无结果。
